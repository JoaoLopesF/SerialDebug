 ...
 * Versions  :
 * ------ 	---------- 		-------------------------
 * 0.9.83	2023-08-17		replace 'byte' by 'uint8_t'
...
void debugHandleInactive()
{
	...
	if (execCount > 0)
	{ // To count executions. You can change it
...
void debugHandleEvent(boolean calledByHandleEvent)
{
	...
	if (diffTime >= DEBUG_MIN_TIME_EVENT)
...
void debugPrintf(boolean newline, const char level, const char *function, const char *format, ...)
{
	...
	if (level != ' ')
	{ // ' ' is used internally in SerialDebug to do printf, if Arduino no have, or for flash F support
...
void debugHandleDebugger(boolean calledByHandleEvent)
{
	...
	for (uint8_t g = 0; g < _debugGlobalsAdded; g++)
	{
		...
		if (process)
		{
			...
			{ // Changed ?
				...
				if (global->pointerOld)
				{ // Only if it has content

					global->changed = true;

					if (_debugSerialApp)
					{ // SerialDebugApp connected ?
			...
			{ // && calledByHandleEvent) { // only if called by handle event
	...
	if (_debugWatchesAdded > 0 && _debugWatchesEnabled)
	{
		...
		for (uint8_t w = 0; w < _debugWatchesAdded; w++)
		{
			...
			if (watch->enabled && (global->updateOldValue || watch->watchCross || watch->operation == DEBUG_WATCH_CHANGED))
			{
				...
				if (!watch->triggered && triggered)
				{
					...
					if (watch->alwaysStop && !alwaysStop)
					{ // Always stop ?
						alwaysStop = true;
					}
				...
				else if (watch->triggered && !triggered)
				{ // Unmark it
		...
		if (totTriggered > 0 && (_debugWatchStop || alwaysStop))
		{ // Stop on watches ?
	...
	if (!hasBreak)
	{ // Not for watch triggered, have a delay for waiting response
		time = (micros() - time);
		if (time >= 250)
		{
...
static int8_t showWatches(String &options, boolean debugSerialApp)
{
	...
	else
	{
		...
		if (strIsNum(options))
		{
			...
			if (num > 0)
			{ // Find by number, else by exact name
...
static boolean findGlobal(const char *globalName, uint8_t *globalNum, boolean sumOne)
{
	...
	for (uint8_t i = 0; i < _debugGlobalsAdded; i++)
	{
		...
		if (_debugGlobals[i].name)
		{ // Memory

			name = _debugGlobals[i].name;
		}
		else if (_debugGlobals[i].nameF)
		{ // For Flash F
...
static void getStrValue(uint8_t type, void *pointer, uint8_t showLength, boolean showSize, String &response, String &responseType)
{
	...
	case DEBUG_TYPE_FLOAT:
		...
		if (responseType)
			responseType = F("float");
		break;
	case DEBUG_TYPE_DOUBLE:
		response = String(*(double *)pointer);
		if (responseType)
			responseType = F("double");
		break;
	...
	case DEBUG_TYPE_INT8_T:
		...
		if (responseType)
			responseType = F("int8_t");
		break;
	case DEBUG_TYPE_INT16_T:
		response = String(*(int16_t *)pointer);
		if (responseType)
			responseType = F("int16_t");
		break;
	case DEBUG_TYPE_INT32_T:
		response = String(*(int32_t *)pointer);
		if (responseType)
			responseType = F("int32_t");
		break;
		// #ifdef ESP32
	...
	case DEBUG_TYPE_UINT8_T:
		response = String(*(uint8_t *)pointer);
		if (responseType)
			responseType = F("uint8_t");
		break;
	case DEBUG_TYPE_UINT16_T:
		response = String(*(uint16_t *)pointer);
		if (responseType)
			responseType = F("uint16_t");
		break;
	case DEBUG_TYPE_UINT32_T:
		response = String(*(uint32_t *)pointer);
		if (responseType)
			responseType = F("uint32_t");
		break;
		// #ifdef ESP32
...
static int8_t showFunctions(String &options, boolean one, boolean debugSerialApp)
{
	...
	for (uint8_t i = 0; i < _debugFunctionsAdded; i++)
	{
		...
		if (_debugFunctions[i].name)
		{ // Memory

			name = _debugFunctions[i].name;
		}
		else if (_debugFunctions[i].nameF)
		{ // For Flash F
		...
		if (show)
		{
			...
			{ // One search ?
				...
				if (_debugFunctions[i].nameF)
				{ // For Flash F - save the content in variable in memory, to not extract Flash again
					_debugString = name;
				}
			...
			{ // Description, not for one search
...
#ifndef BOARD_LOW_MEMORY // Not for low memory boards
					if (_debugFunctions[i].description)
					{ // Memory
	...
	if (!debugSerialApp)
	{
		...
		if (!one && showed > 0)
		{
			...
			if (!_debugRepeatCommand && !debugSerialApp)
...
static void callFunction(String &options)
{
	...
	printSerialDebug();
	if (_debugFunctions[num].name)
	{ // Memory
		PRINTF(F("Calling function %u -> %s("), (num + 1), _debugFunctions[num].name);
	}
	else if (_debugFunctions[num].nameF)
	{ // Use a temporary var to not get flash again
...
#ifndef BOARD_LOW_MEMORY // Not for low memory boards
					if (_debugGlobals[i].description)
					{ // Memory
